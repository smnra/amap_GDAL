...
class GetMeituan():
    ...
    def readCurr(self):
        ...
        if not os.path.exists(r'./searchPoiNogps.csv'):
            with open(r'./searchPoiNogps.csv', mode='w', encoding='utf-8', errors='ignore') as f:
                f.write( "searchCity,searchName, searchID, " +   "poiCity, poiName, poiId, poiType, poiArea, url, poiUrl, poiSource, poiComment, poiPrice, poiAddress" + '\n')
        ...
        for searchName in self.poiSearch[currUrlIndex:-1]:
            ...
            while not isLastPage:
                ...
                for div in aTags:
                    ...
                    poiInfo = searchList + [poiCity, poiName, poiId, poiType, poiArea, url, poiUrl, poiSource, poiComment,
                               poiPrice, poiAddress + '\n']
                    print(poiInfo)
    ...
    def requestsSearch(self,city,name):
        self.poiSearch = self.readUrlList(r'./poiSearchName.txt')
        currUrlIndex = self.readCurr()   #读取进度
